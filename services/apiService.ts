 ...
 * NOT the Breeze Proxy directly and NOT window.location.origin.
 */
const BASE_URL = "https://market-attribution-backend-919207294606.us-west1.run.app"; 
...
export const setBreezeSession = async (apiSession: string) => {
  const res = await fetch(`${BASE_URL}/api/breeze/admin/api-session`, {
    method: "POST",
    headers: getHeaders(),
...
export const fetchQuote = async (symbol: string) => {
  const res = await fetch(`${BASE_URL}/api/breeze/quote`, {
    method: "POST",
    headers: getHeaders(),
    body: JSON.stringify({ symbol }),
  });
  const data = await handleResponse(res);
  if (!data.Success || data.Success.length === 0) throw new Error(`No quote data for ${symbol}`);
  return data.Success[0];
};

export const fetchDepth = async (symbol: string) => {
  const res = await fetch(`${BASE_URL}/api/breeze/depth`, {
    method: "POST",
    headers: getHeaders(),
    body: JSON.stringify({ symbol }),
  });
  const data = await handleResponse(res);
  if (!data.Success || data.Success.length === 0) throw new Error(`No depth data for ${symbol}`);
  return data.Success[0];
};

export const fetchHistorical = async (symbol: string, fromDate: string, toDate: string) => {
  const res = await fetch(`${BASE_URL}/api/breeze/historical`, {
    method: "POST",
    headers: getHeaders(),
...
export const analyzeMarketRadar = async (log: MarketLog): Promise<NewsAttribution> => {
  const res = await fetch(`${BASE_URL}/api/gemini/market-radar`, {
    method: "POST",
    headers: getHeaders(),
...
export const analyzeStockDeepDive = async (symbol: string): Promise<NewsAttribution> => {
  const res = await fetch(`${BASE_URL}/api/gemini/stock-deep-dive`, {
    method: "POST",
    headers: getHeaders(),
    body: JSON.stringify({ symbol }),
  });
