...
import { X, ArrowUp, ArrowDown, Bookmark, RefreshCw, Activity, AlertCircle, Loader2 } from 'lucide-react';
      ...
      try {
        ...
        if (quote) {
          supabase
            .from('priority_stocks')
            .update({
              last_price: parseFloat(quote.ltp),
              change_val: parseFloat(quote.change),
              change_percent: parseFloat(quote.ltp_percent_change),
                    ...
                    {quote ? (
                      <div className="text-right">
                        <p className="text-xs font-black text-slate-900 tabular-nums">
                          {parseFloat(quote.ltp).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                        </p>
                        <div className={`flex items-center justify-end gap-1 text-[8px] font-black ${isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
                          ...
                          <span className="tabular-nums">{Math.abs(parseFloat(quote.ltp_percent_change)).toFixed(2)}%</span>
                        </div>
                      </div>
                    ) : (
                      <div className="text-right">
