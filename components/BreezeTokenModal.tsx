...
const BreezeTokenModal: React.FC<BreezeTokenModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setProxyUrl(localStorage.getItem('breeze_proxy_url') || 'https://breeze-proxy-919207294606.us-west1.run.app');
    }
  }, [isOpen]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!health.ok) {
        throw new Error(`Proxy Health Check Failed: ${health.error}`);
      }

      // If session is provided, try to set it
      if (apiSession) {
        await setDailyBreezeSession(apiSession, proxyKey);
        setMessage('Session updated successfully!');
      } else {
        setMessage('Settings saved (no session update).');
      }
      ...
      setStatus('error');
      setMessage(e.message || 'Failed to update session');
    }
  };
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200">
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            />
            <p className="text-xs text-slate-400 mt-1">Obtain this from the Breeze login flow.</p>
          </div>
          ...
          <div className="pt-4 border-t border-slate-100">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-slate-500 mb-1">Proxy URL</label>
                <input
                  type="text"
                  value={proxyUrl}
              ...
              <div>
                <label className="block text-xs text-slate-500 mb-1">Admin Key (X-Proxy-Key)</label>
                <input
                  type="password"
                  value={proxyKey}
          ...
          {message && (
            <div className={`p-3 rounded-lg text-sm flex items-center ${status === 'error' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>
              {status === 'error' ? <ShieldCheck className="w-4 h-4 mr-2" /> : <ShieldCheck className="w-4 h-4 mr-2" />}
              {message}
            </div>
          )}
